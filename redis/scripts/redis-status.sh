...
case $METRIC in
    ...
    'changes_since_last_save')
        cat $CACHE | grep "changes_since_last_save:" | cut -d':' -f2
        ;;
    'bgsave_in_progress')
        cat $CACHE | grep "bgsave_in_progress:" | cut -d':' -f2
        ;;
    'bgrewriteaof_in_progress')
        cat $CACHE | grep "bgrewriteaof_in_progress:" | cut -d':' -f2
        ;;
    'aof_enabled')
    ...
    'instantaneous_ops_per_sec')
        ...
        ;;
    'blocked_clients')
        cat $CACHE | grep "blocked_clients:" | cut -d':' -f2
        ;;
    'expired_keys')
    ...
    'list_key_space_db')
        ...
        ;;                                                     
    'expires')
        cat $CACHE | grep "db" | cut -d',' -f2 | cut -d'=' -f2
        ;;
    'keys')
        cat $CACHE | grep "db" | cut -d',' -f1 | cut -d'=' -f2
        ;;
    *)   
